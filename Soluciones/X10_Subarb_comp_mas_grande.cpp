...
/* Considero el caso peor, árbol degenerado
0 <= n < 2, T(n) = k0*n^0 
...
std::tuple<int, int, bool> subArbCompMasGrandeAux(BinTree<char> const& t) {
    if (t.empty()) return { 0, 0, true };
        ...
        int altSubMax_c = 1;
        //O conecta el árbol, o no lo hace. En el caso q no, el único subarbol conectable más grande posible es el mínimo de ambos lados conectables
