...
public:
    ...
    void attach(Node* target, Node* node);
...
template<typename T>
void ListLinkedDouble<T>::attach(Node* target, Node* node) {
...
template<typename T>
void ListLinkedDouble<T>::rotar_derecha() {
    if (this->num_elems > 2) {
        Node* curr = this->head->next->next->next;
        Node* prevImp = this->head->next;
        ...
        this->attach(this->head->next, prevImp); //Cerramos la cadena
    }
}
