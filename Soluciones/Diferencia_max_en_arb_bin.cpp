...
/*
Explicación breve:
Para cada nodo, calcula la diferencia entre el nodo actual y sus hijos, actualiza la máxima 
diferencia y propaga el último valor visitado. Finalmente, devuelve la máxima diferencia global.

Coste:
Es un recorrido en preorden, luego tiene coste O(n) en los n nodos del árbol
*/

//int1: max_diferencia
//int2: valor previo
std::pair<int, int> maxima_diferencia_aux(BinTree<int> const& t) {
    int max_dif = 0, prev = t.root();

    auto [max_dif_L, prev_L] = maxima_diferencia_aux(t.left());
    if (!t.left().empty()) {
        max_dif = std::(max_dif_L, abs(t.root()-t.left().root()));
        prev = prev_L;
    }

    auto [max_dif_R, prev_R] = maxima_diferencia_aux(t.right());
    if (!t.right.empty()) {
        max_dif = std::max({ max_dif, abs(t.root.right() - prev_L), max_dif_R});
        prev = prev_R;
    }
...
int maxima_diferencia(BinTree<int> const& t) {
    auto [max, prev] = maxima_diferencia_aux(t);
    return max;
}
...
void res() {
    ...
    std::cout << "WITH PREORDER FUNCTION:\n";
    bool second = false;
    int prev = t.root(), max = -1;
    t.preorder([prev&, max&, second&](int x) {
        if (!second) second = true;
        else {
            max = std::max(max, abs(prev - x));
            prev = x;
        }
        });
    std::cout << max << '\n';
    std::cout << "WITH AUXILIARY FUNCTION:\n";
    std::cout << maxima_diferencia(t) << '\n';
}
...
if (t.empty()) return { 0,0 };
    ...
    auto [max_dif_L, prev_L] = maxima_diferencia_aux(t.left());
    if (!t.left().empty()) {
        int dif_root_and_L = abs(t.root() - t.left().root());
        max_dif = std::max(dif_root_and_L, max_dif_L);
        prev = prev_L;
    }

    auto [max_dif_R, prev_R] = maxima_diferencia_aux(t.right());
    if (!t.right().empty()) {
        int dif_root_and_R = abs(prev - t.right().root());
        max_dif = std::max({ max_dif, dif_root_and_R, max_dif_R });
        prev = prev_R;
    }
...
[&second, &prev, &max](int x) {
    if (!second) second = true;
    else {
        max = std::max(max, abs(x - prev));
        prev = x;
    }
}